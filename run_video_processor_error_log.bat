...
REM Video Processor Runner for Windows - Error Log Only
REM Script nay chay tu thu muc goc cua du an

echo ================================
echo    VIDEO PROCESSOR RUNNER - ERROR LOG
echo ================================
echo.
...
echo ================================
echo.
echo 1. Chay video tu folder mac dinh (chi hien thi loi)
echo 2. Chay video tu folder tuy chon (chi hien thi loi)
echo 3. Thoat
echo.
...
echo [INFO] Chi hien thi thong bao loi neu co...
echo.

REM Run and capture errors only
python run\all_in_one.py 2>&1 | findstr /C:"[ERROR]" /C:"ERROR" /C:"Exception" /C:"Traceback" /C:"Failed" /C:"Error"
if errorlevel 1 (
    echo [INFO] Xu ly video thanh cong! (khong co loi)
) else (
    echo [ERROR] Co loi xay ra khi xu ly video!
)
goto end_script
...
REM Run and capture errors only
python run\all_in_one_temp.py 2>&1 | findstr /C:"[ERROR]" /C:"ERROR" /C:"Exception" /C:"Traceback" /C:"Failed" /C:"Error"
if errorlevel 1 (
    echo [INFO] Xu ly video thanh cong! (khong co loi)
) else (
    echo [ERROR] Co loi xay ra khi xu ly video!
